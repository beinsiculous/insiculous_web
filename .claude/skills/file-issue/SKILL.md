...
(the clone is `fortresskey/` until Name Drop's app phase lands);
...
`fortknight` (still `fortresskey` on GitHub until Name Drop's app phase renames it) has the
workflow but it fails until its `ADD_TO_PROJECT_PAT` secret is set.
