...
- The site has one face (`docs/app.md`): FortKnight (`/fortknight/`), plus the shared `/profile/` (the studio's skin and chrome, not a face's) and a landing at `/`. Its nav is six pills — **Overview · Keep · Fork Knife · Fresh Keep · Folk Knowledge · Achievements** — one per Name Drop stone with a page, and **three of them 404 today**: Fork Knife, Fresh Keep and Folk Knowledge have no page until `beinsiculous/insiculous_web#18`, `#19` and `#20` land. No gate catches that (nothing crawls nav hrefs; the visual gates walk `dist/`), so **`dev` → `main` is held until those three exist.** Six is itself interim — Name Drop has eight stones, and `#23` takes the nav to ten.
