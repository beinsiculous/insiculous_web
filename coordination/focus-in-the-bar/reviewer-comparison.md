...
| batch 3 code | claude (planner) | 3 | 3 | 0 | 0 | C1: "`DIALOG_ROUTES` now holds two entries that open no dialog … the paragraph that said *why* the dialog is audited here … was deleted rather than extended" — a generalisation that kept the old name and dropped the old reason | — |
| batch 3 code | kimi | 2 | 2 | 0 | 0 | F2: "a future `.nav-peripheral { display: … }` rule silently defeats the `hidden` attribute (author rules beat the UA `[hidden]` rule), leaving an empty strip behind a visible pill — nothing fails" — the one branch no fixture reaches | 6m53s (14:56:10 → 15:03:03) |
