...
```json
{
  ...
  "commitments": [
    {"title": "Piano lesson", "repeats": "every week", "weekdays": ["friday"], "start": "2:00 PM", "lasts": "2 h 15 min", "category": "spirituality-development", "from": "'Piano w/ the tutor' — 12 occurrences seen"},
...
3. The reply is validated (same rules as `check_import_document`), shown as a review table
   (done today, read-only: *Apply from assistant* on `/fortknight/assistant/` — built on the parked chain, at the tag `creation-chain-parked`; Fork Knife's own assistant page went with the `/forkknife/`
