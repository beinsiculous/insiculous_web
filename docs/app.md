...
- **One face live, one profile** (`src/lib/faces.js`, `FaceLayout.astro`): **FortKnight** under `/fortknight/`
  ...
  belongs to neither, so it takes the studio's: it is a `BaseLayout` page whose look lives in the
  site's own `src/styles/global.css`, with its planner widgets styled by `src/styles/app-widgets.css`
  (`themes.css` is mirrored from this repo and knows only the one
  ...
  the profile *as stored at save time* (`pickAnswers` in `weights-rules.js` over the key sets the chain's
  `answer-keys.js` named — deleted on `main` with the rest of the chain, preserved at the tag
  `creation-chain-parked`: Spoon Feed owned `mealPlan`, Build owned `mealPlan` + `standingAppointments` + `tasks`,
  Fork Knife's questionnaire owned `FORKKNIFE_ANSWER_KEYS`, FortKnight's owned the rest), so none wipes another's,
...
- FortKnight pages (the parked design — the live Overview and day pages are the seed-fed ones of the previous section): `/fortknight/` **Overview** — the fortnight grid + date resolver (navigates to the resolved day) and, below it,
  ...
  `meals`; the page's bespoke kinds go in as its slot; `src/lib/question-fields.js` reads/populates the simple kinds
  — both deleted on `main` with the rest of the chain, preserved at the tag `creation-chain-parked`), a profile line above the form naming the active profile and whether it is saved (several
...
- Fork Knife pages (the parked design — these routes are removed from the live site) (`docs/meal-plan.md`): `/forkknife/` **Overview** — the fortnight menu as the 14-day grid
  (`components/MenuDayCard.astro`, deleted on `main` with the rest of the chain and preserved at the tag
  `creation-chain-parked`: one line per meal per day from `menuForDay`, leftovers marked, rotated to the
...
- Static hosting: the face is a path under `beinsiculous.com/`, sharing the origin with the Be Insiculous
  ...
  (`wrangler.toml`) by the site repo's GitHub Actions workflow on every push to its `main` (production) or
  `dev` (the staging environment) — type-check and build
