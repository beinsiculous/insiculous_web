---
title: 'Insiculous Invaders'
blurb: 'Space Invaders with destructible barriers, UFO bonus runs, and true two-cannon co-op.'
status: 'playable'
wasm: '/games/invaders/v2/game.js'
editor: '/playground/invaders/v1/game.js'
screenshots: []
order: 3
---

The marching formation, the crumbling barriers, the UFO streaking across the
top for bonus points — the full Invaders loop, with an actual win state when
the last alien falls.

Co-op is the real draw: two cannons on screen with independent lives and
scores, defending the same barriers against the same descending wave.
Eleven achievements, chaos modes included.

Built on Insiculous 2D. Playable right here in the browser (WebGPU —
Chrome/Edge, or Firefox with `dom.webgpu.enabled`); desktop builds
(Vulkan / Metal / DX12) run the same code natively.
