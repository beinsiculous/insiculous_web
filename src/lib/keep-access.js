...
//
// This module exists because a keep-fed page's boot sequence is the same everywhere — read the stored
...
// different ending from a VALIDATION refusal. Storage that will not parse is not "no keep":
...
// forever.
//
// A keep that reads fine but fails validation is the opposite case, and is KEPT. A refusal means the
// document is intact but not drawable — wrong file, newer format, no days — and none of those is
// wreckage. The sharpest case is a newer Fort Knight export: the remedy is a website deploy, which the
// person holding the phone cannot do, and a keep forgotten at boot would be gone by the time the update
// shipped. Keeping costs nothing: the keep stays deletable, from Keep's Forget button or /profile/.
import { validateKeep } from "./keep.js";
import { KEEP_STORE_KEY, clearKeep } from "./keep-store.js";

/** The stored keep, adjudicated. One of:
 *    { status: "keep", keep }  — validated by the same validateKeep a picked file goes through;
 ...
 *    { status: "kept", reason }     — read fine, but the validator refuses to draw it; the keep is NOT
...
export function readStoredKeep() {
  ...
  // A stored literal `null` parses perfectly and is not a keep. Treated as unreadable rather than as
  ...
  if (!readable || parsed === null || parsed === undefined) {
    clearKeep();
    return { status: "cleared", reason: "The stored keep could not be read, so it has been forgotten. Load it again." };
  }
  ...
  return { status: "keep", keep: check.keep };
}
