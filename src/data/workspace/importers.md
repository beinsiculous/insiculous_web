...
`normalizeImportDocument()` in `app/shared/import-document.js` (Python:
...
- **Assistant page, Apply from assistant** — paste or upload the document and Apply
  ...
  (`renderImportReview()` in `app/shared/import-review.js`): the commitments and tasks as readable rows,
