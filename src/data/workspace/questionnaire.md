...
- Question definitions, slider bounds and derivation constants: `data/questionnaire.json`
  ...
  questions elsewhere.
- Derivation rule: `scripts/fk_core/weights.py` (Python) and `app/shared/weights-rules.js`
...
- App: `/fortknight/questionnaire/` in the Astro app renders the sections without a `face`; ForkKnife's questionnaire
  ...
  plus the menu editor (`docs/meal-plan.md`); `/profile/` holds the profile actions and the workspace downloads. Each
  questionnaire *is* its face's settings; both write the same profile, each only its own answer keys
  (`FORKKNIFE_ANSWER_KEYS`), over the profile as stored at save time. Named profiles per device, one active (`docs/app.md` → User settings): Save derives
...
|---|---|---|
| **Startup** | 1 group size · 2 existing systems — commitments (standing appointments incl. work/school hours) and tasks, one at a time, plus what the Assistant page applied | who this is for; the person's commitments (anchors the blocks are cut around) and tasks (day pages) |
...
(6 is Startup Q2 and is not asked twice. 1–2 live in "Your day and year", 3 is Startup Q2's
commitments editor, 4 in "Your week", 5 at the end of the Focus questions.)
...
`fk_core/astronomy.py` / `app/shared/astronomy.js` hold the Meeus formulas at day precision (ΔT by the
